    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/api/shop/file/brandstory-multiImgUpload?TOKEN='+sessionStorage['TOKEN'];
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
		...
		oAjax.header("token",sessionStorage['TOKEN']);
...
// 			sCallback : location.href.replace(/\/[^\/]*$/, '') + '/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
 			sUrl : '/api/shop/file/brandstory-upload?TOKEN='+sessionStorage['TOKEN'],								//파일업로드를 처리하는 페이지
 			sCallback : '/res/smarteditor2/popup/photo_uploader/callback.html',		//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
